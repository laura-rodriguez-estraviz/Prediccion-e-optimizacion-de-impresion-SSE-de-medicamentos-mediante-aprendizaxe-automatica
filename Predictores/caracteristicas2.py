...
while iteration < N_ITER and iteration < 375888 : #para no numero maximo de combinacións en caso de alcanzalo
    ...
    filter_ok = random.choice([True, False])
    ...
    X_sub, y_sub = get_Xy(sub, use_cols)

    for train_idx, val_idx in skf.split(X_sub, y_sub):
        fold_train = sub.iloc[train_idx]
...
#gardo os resultados
results_df.to_csv("caracteristicas2.csv", index=False)
...
if best["filter_ok"]:
    subtrain = subtrain[subtrain[TARGET] != "ok"]
    subtest = subtest[subtest[TARGET] != "ok"]
...
if best["filter_ok"]:
    subtrainTotal = subtrainTotal[subtrainTotal[TARGET] != "ok"]
...
}, "caracteristicas2.pkl")

print("\nModelo gardado en caracteristicas2.pkl")
print("Resultados da busca gardados en  caracteristicas2.csv")